...
GENERATION_UNAVAILABLE_CONTACT_ADMIN_TEXT = "失败：当前无法完成生成，请联系管理员处理。"
GENERATION_UNAVAILABLE_REFUNDED_TEXT = "生图失败已退款：当前无法完成生成，请联系管理员处理。"
...
def customer_failure_text(exc: Exception) -> str:
    ...
    if isinstance(exc, UpstreamError):
        if exc.status == 402:
            return GENERATION_UNAVAILABLE_CONTACT_ADMIN_TEXT
...
def sanitize_customer_note_text(text: str | None) -> str | None:
    ...
    if text.startswith("生图失败退款"):
        if "402" in upper or "PAYMENT" in upper or "余额" in text or "额度" in text:
            return GENERATION_UNAVAILABLE_REFUNDED_TEXT
        if "超时" in text or "TIMEOUT" in upper:
